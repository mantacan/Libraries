...
namespace s21 {
TEST(Suite_Stack, DefaultConstructor) {
...
TEST(Suite_Stack, InitializerListConstructor) {
...
TEST(Suite_Stack, InitializerListConstructor_Pair) {
...
TEST(Suite_Stack, EmptyInitializerListConstructor) {
  s21::Stack<int> s21_s{};
  ASSERT_TRUE(s21_s.empty());
  ASSERT_EQ(s21_s.size(), 0u);
}

TEST(Suite_Stack, EmptyPair) {
...
TEST(Suite_Stack, PushPopString) {
...
TEST(Suite_Stack, CopyConstructorString) {
...
TEST(Suite_Stack, CopyConstructorPair) {
...
TEST(Suite_Stack, CopyConstructorEmpty) {
  std::stack<int> std_stack;
  s21::Stack<int> s21_stack;
  std::stack<int> std_copy(std_stack);
  s21::Stack<int> s21_copy(s21_stack);
...
TEST(Suite_Stack, MoveConstructorPair) {
  s21::Stack<std::pair<std::string, int>> s1;
  std::stack<std::pair<std::string, int>> s1_stl;
  s1.push({"one", 1});
  s1_stl.push({"one", 1});
...
TEST(Suite_Stack, MoveConstructorString) {
...
TEST(Suite_Stack, CopyAssignmentOperator) {
  ...
  my_copy_stack = std::move(my_stack);
...
TEST(Suite_Stack, MoveAssignmentOperator) {
  s21::Stack<int> my_stack;
  std::stack<int> stl_stack;
  for (int i = 0; i < 10; ++i) {
    my_stack.push(i);
...
TEST(Suite_Stack, MoveAssignmentOperatorPair) {
  s21::Stack<std::pair<std::string, int>> s1;
  std::stack<std::pair<std::string, int>> s1_stl;
  s1.push({"one", 1});
  s1_stl.push({"one", 1});
...
TEST(Suite_Stack, AssignmentOperatorEmpty) {
  std::stack<int> std_stack;
  s21::Stack<int> s21_stack;
  std::stack<int> std_copy;
  s21::Stack<int> s21_copy;
  std_copy = std_stack;
  s21_copy = std::move(s21_stack);
  EXPECT_EQ(std_copy.size(), s21_copy.size());
  EXPECT_EQ(std_stack.size(), s21_stack.size());
}

TEST(Suite_Stack, InsertManyFrontTest) {
